...
export async function extract(): Promise<Record<string, string>> {
  ...
  for (const region of regions) {
    ...
    const setCodeRewrites: Record<string, string> = region === "" ? {
      "WotC Promos": "PR",
      "Base Set 2": "B2",
      "Expedition": "EX",
      "Best of Game": "BP",
    ...
    $("table.sets-table > tbody > tr").each((_, e) => {
      ...
      let setName = setCodeElement.html()?.match(/<img.*?set.*?>(.*?)<span/m)?.[1];
      setName = setName != null ? cheerio.load(setName).text().trim() : undefined;
      const setCode = setName != null && setName in setCodeRewrites ? setCodeRewrites[setName] : setCodeElement.find(".code").text()?.trim();
    ...
    const proms = setCodes.map(async (setCode) => {
      ...
      $(".card-classic").each((_, e) => {
        ...
        const cardSetNumber = cardElement.find(".card-set-info").text().split("#", 2)[1]?.trim();
